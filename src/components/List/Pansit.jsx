...
export const Pansit = () => {
  const desiredCategories = ["pansit"]; // Add the desired category names to filter

  const filteredMenu = useMemo(() => {
    return MenuList.filter((category) =>
      desiredCategories.includes(Object.keys(category)[4])
    );
  }, [desiredCategories]);
  ...
  return (
    <div>
      {filteredMenu.map((category) => {
        const categoryName = Object.keys(category)[4];
