...
export const FreshFruitShakes = () => {
  const desiredCategories = ["freshfruitshakes"]; // Add the desired category names to filter

  const filteredMenu = useMemo(() => {
    return MenuList.filter((category) =>
      desiredCategories.includes(Object.keys(category)[7])
    );
  }, [desiredCategories]);
  ...
  return (
    <div>
      {filteredMenu.map((category) => {
        const categoryName = Object.keys(category)[7];
