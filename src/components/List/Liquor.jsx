...
export const Liquor = () => {
  const desiredCategories = ["liquor"]; // Add the desired category names to filter

  const filteredMenu = useMemo(() => {
    return MenuList.filter((category) =>
      desiredCategories.includes(Object.keys(category)[8])
    );
  }, [desiredCategories]);
  ...
  return (
    <div>
      {filteredMenu.map((category) => {
        const categoryName = Object.keys(category)[8];
