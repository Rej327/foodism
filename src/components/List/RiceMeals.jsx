...
export const RiceMeals = () => {
  const desiredCategories = ["ricemeals"]; // Add the desired category names to filter

  const filteredMenu = useMemo(() => {
    return MenuList.filter((category) =>
      desiredCategories.includes(Object.keys(category)[3])
    );
  }, [desiredCategories]);
  ...
  return (
    <div>
      {filteredMenu.map((category) => {
        const categoryName = Object.keys(category)[3];
